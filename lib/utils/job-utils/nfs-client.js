    ...
    NfsClient.prototype.mount = function(){
        ...
        return Promise.resolve().then(function(){
            childProcess.exec(
                ...
                function(mkdirError, stdout, stderr){ // jshint ignore:line
                    if(mkdirError){
                        throw mkdirError;
                    }
                    logger.debug('mount stdout: ' + stdout + " stderr: " + stderr); // jshint ignore:line
                }
            );
        }).then(function(){
            childProcess.exec(
                'mount ' + self.shareIP + ':' + self.shareName + ' ' + self.mountDir,
                function(mountError, stdout, stderr){ // jshint ignore:line
                    if(mountError){
                        throw mountError;
                    }
                    logger.debug('mount stdout: ' + stdout + " stderr: " + stderr); // jshint ignore:line
    ...
    NfsClient.prototype.umount = function(){
        ...
        return new Promise(function(resolve, reject){
            childProcess.exec(
                ...
                function(umountError, stdout, stderr){ // jshint ignore:line
                    if(umountError){
                        reject(umountError);
                    }else{
                        resolve(stdout);
                        logger.debug('umount stdout: ' + stdout + " stderr: " + stderr); // jshint ignore:line
                    }
                }
            );
        });
